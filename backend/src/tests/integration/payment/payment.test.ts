...
describe('Payment API - Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const invRes = await request(app)
      ...
      .send({
        customerId,
        notes: 'TEST_Payment',
        items: [{ serviceId, quantity: 1, unitPrice: 10000, description: 'TEST_Payment' }],
      });
  ...
  describe('POST /api/v1/payment', () => {
    ...
    it('should accept all payment methods', async () => {
      const methods = ['CASH', 'BANK_TRANSFER', 'UPI', 'CREDIT_CARD', 'DEBIT_CARD', 'PAYPAL', 'OTHER'];
      
      for (const method of methods) {
        const res = await request(app)
          .post('/api/v1/payment')
          .set('Cookie', authCookie)
          .send({ invoiceId, amount: 100, paymentMethod: method, notes: 'TEST_Payment' });
        expect(res.status).toBe(201);
      }
    });
  });
