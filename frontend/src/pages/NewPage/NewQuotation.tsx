...
export default function NewQuotation() {
  ...
  const validate = (): boolean => {
    ...
    const dataToValidate = {
      ...
      items: formData.items
        .filter(item => item.serviceId) // Only validate items with service selected
        .map(item => ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submitData: CreateQuotationDto = {
      ...
      termsConditions: formData.termsConditions || undefined,
      items: formData.items
        .filter(item => item.serviceId) // Only submit items with service selected
        .map(item => ({
          serviceId: item.serviceId,
          description: item.description || undefined,
          quantity: Number(item.quantity),
    ...
    createQuotation(submitData, {
      onSuccess: (data) => {
        navigate(`/quotation/${data.data.id}`);
      },
    });
  };
