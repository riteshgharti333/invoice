...
import type { CreateQuotationDto, UpdateQuotationDto } from '@invoice/shared/types';
...
export function useSearchQuotations(params: { q?: string; cursor?: string }) {
  return useQuery({
...
export function useFilterQuotations(params: Record<string, string> & { cursor?: string }) {
...
export function useUpdateQuotation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateQuotationDto }) => quotationApi.update(id, data),
...
export function useUpdateQuotationStatus() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) => quotationApi.updateStatus(id, status),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: quotationKeys.lists() });
      queryClient.invalidateQueries({ queryKey: quotationKeys.detail(variables.id) });
